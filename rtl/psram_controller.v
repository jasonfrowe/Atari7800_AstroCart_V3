...
`ifdef VERILATOR
reg [15:0] v_mem [0:65535];
reg [2:0] v_busy_cnt;

assign busy = (v_busy_cnt != 3'd0);
...
always @(posedge clk or negedge resetn) begin
    if (!resetn) begin
        ...
        if (v_busy_cnt != 3'd0) begin
            v_busy_cnt <= v_busy_cnt - 3'd1;
        end else if (read) begin
            dout <= v_mem[addr[16:1]];
            v_busy_cnt <= 3'd2;
        end else if (write) begin
            if (byte_write) begin
                if (addr[0])
                    v_mem[addr[16:1]][15:8] <= din[15:8];
                else
                    v_mem[addr[16:1]][7:0] <= din[7:0];
            end else begin
                v_mem[addr[16:1]] <= din;
            end
            v_busy_cnt <= 3'd2;
